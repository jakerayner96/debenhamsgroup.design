...
**Quick add** is a page-level setting, never per card (08 Sep): the strip has **On / Off** (every product or none) and a **style** that applies to every product — `image` = bag-with-plus on the photo under the wishlist, taking the wishlist button's shape (chip on PLT/MAN new, square on KM, circle on Debenhams); `button` = the full-width "Add" between image and titles (MAN live); `mini` = 24px primary-styled "Add" pill top-right of the info block, on the titles row (Adanola Complete-The-Look reference) — the price row is never squeezed so it never wraps; zero card-height cost, hidden with the info block in the tile view. Both default to the fascia config until touched; `?qa=off|image|button|mini` on plp.html. Legacy config values `icon`/`chip` mean `image`.
